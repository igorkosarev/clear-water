import type { BOMLine, RankedRecommendation } from '@/types'
import modules from '@/data/modules.json'
import suppliers from '@/data/suppliers.json'

type RawModule = { id: string; nameKey: string; descriptionKey: string; type: string; removes: string[]; costUSD: number; diyDifficulty: string; materials: string[] }
type RawSupplier = { id: string; name: string; country: string; url: string; modules: string[] }

export function generateBOM(recommendation: RankedRecommendation): BOMLine[] {
  const allModules = modules as RawModule[]
  const allSuppliers = suppliers as RawSupplier[]

  return recommendation.template.modules.map(moduleId => {
