...
import { ContaminantDetailModal } from './ContaminantDetailModal'
...
export function RemainingBadges({ remainingContaminants }: RemainingBadgesProps) {
  const { t } = useTranslation()
  const [selected, setSelected] = useState<Contaminant | null>(null)

  if (remainingContaminants.length === 0) return null

  return (
    <>
      <div>
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">
          {t('result.remaining.label')}
        </p>
        <div className="flex flex-wrap gap-2">
          {remainingContaminants.map(id => {
